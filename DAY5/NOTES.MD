Search - it invovles in a agent that is given an initial state and a goal state, and it returns a solution of hoe to get from the former of the latter A navigator app uses a typical search process, where the agent (the thinking part of the program) receives as input your current location and your desired destination, and, based on a search algorithm, returns a suggested path. However, there are many other forms of search problems, like puzzles or mazes.
...
solution 
a sequence of action that leads from the intial state to goal state 
optimal solution 
a solution that has lowest path cost among all solutions 
nodes - it contains all the information that makes them very useful for the purpose of search algorithms 
